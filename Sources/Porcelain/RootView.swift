...
private struct SidebarView: View {
    ...
    var body: some View {
        ...
        .navigationTitle("Porcelain")
        .searchable(text: $appModel.searchText, placement: .sidebar)
        .toolbar {
            ToolbarItem {
                addRepositoryMenu
            }
    ...
    private var repositorySelection: Binding<Repository?> {
        Binding {
            appModel.selectedRepository
        } set: { repository in
            if let repository, repository != appModel.selectedRepository {
                appModel.select(repository)
            }
        }
    }

    private var addRepositoryMenu: some View {
        Menu {
            Button {
